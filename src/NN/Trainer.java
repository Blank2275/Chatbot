...
public class Trainer {
    ...
    public Network train(int generations, DataSet data){
        if(data.getInput()[0].length != base.getLayers()[0].getN() || data.getOutput()[0].length != base.getLayers()[base.getLayers().length - 1].getN()){
            //invalid input data
            System.out.println("Error training, invalid data");
            return null;
        }
    ...
    private double calcError(double[][] expected, double[][] actual) {
        ...
        for (int i = 0; i < expected.length; i++) {
            for (int j = 0; j < expected[0].length; j++) {
                diff += Math.abs((3 * expected[i][j] + 1) * (expected[i][j] - actual[i][j]));
                total++;
            }
        }
